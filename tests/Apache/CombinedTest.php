...
class CombinedTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $parser = new \Kristuff\Parselog\ApacheAccessLogParser('%h %l %u %t "%r" %>s %O "%{Referer}i" "%{User-Agent}i"');
    ...
    {
        $parser = new \Kristuff\Parselog\ApacheAccessLogParser('%h %l %u "%r" %>s %O "%{Referer}i" "%{User-Agent}i"');
